...
In this problem we work with i.i.d. samples X1,…,Xn from an exponential distribution Exp(λ), whose mean is E[X]=1/λ=θ.
For this task, the true parameter is set to θ=id_num/10=1.7.
The sample mean Xˉ is a natural estimate of θ.

The goal of the task is to compare four different ways of constructing a confidence interval for θ, and to check using simulations whether these intervals actually contain the true parameter with probability close to the chosen confidence level 1−α.
To study this properly, we vary the sample size n∈{5,10,30,100}, the confidence level α∈{0.1,0.05,0.01}, and the number of repetitions m∈{100,1000,5000}.
...
ci1_theta <- function(x, alpha) {
  n <- length(x)
  xbar <- mean(x)
  chi_low <- qchisq(alpha / 2, df = 2 * n)       # χ²_{α/2, 2n}
  chi_up  <- qchisq(1 - alpha / 2, df = 2 * n)   # χ²_{1-α/2, 2n}
...
```{r}
# 2: normal approximation with known variance (uses true θ)
...
```{r}
# 3: rearranged inequality (no θ inside)
...
simulate_one <- function(n, alpha, m_reps) {
  cover <- numeric(4) #cover[i] counts how many times method contains the true θ
...
```{r}
#runs the simulation for all (n, α, m)
results <- data.frame()
...
The goal of the simulation was to check whether each confidence interval method contains the true parameter θ=1.7 with probability approximately equal to the nominal level 1−α

Across all sample sizes and confidence levels, the results show:

Method 1— Highly accurate

-   Coverage is consistently very close to the target value.\
    For example:

    -   α = 0.10: coverage ≈ 0.90–0.93

    -   α = 0.05: coverage ≈ 0.94–0.97

    -   α = 0.01: coverage ≈ 0.98–1.00

        This holds even when **n = 5**, confirming that Method 1’s exactness does not depend on asymptotics.

**Method 2** — Acceptable, improves with n

-   Overcoverage appears for small n (n=5, α=0.10 → 0.92–0.93 rather than 0.90).

-   But for n ≥ 30 the coverage is close to nominal.

**Method 3 (Rearranged inequality, corrected method)** — Behaves exactly like Method 2 in coverage
...
-   n = 10, α=0.10 → 0.9086 (Method 2) vs 0.9086 (Method 3)

-   n = 30, α=0.05 → 0.9466 (Methods 2 & 3)

-   n = 100, α=0.05 → 0.9516 (both methods)

The one place where Method 3 fails is when n≤z1−α/2

This happens only when (n=5, α=0.01), and Method 3 produced no intervals at all, giving coverage = 0.
In this case the CI simply cannot exist.

**Method 4** — Slight undercoverage for small n, accurate for larger n

-   For n=5: coverage ranges 0.83–0.89 (slightly below the target).

-   For n=10: coverage improves to \~0.90–0.95.

-   For n ≥ 30: coverage stabilizes and closely matches the nominal level.
...
It relies on the exact distribution of 2λnXˉ, which follows a chi-square distribution with 2n degrees of freedom.
For Method 1 we use the fact that if X_i \~ Exp(λ), then sum(X_i) has a Gamma(n, λ) distribution, and therefore 2 λ sum(X_i) \~ χ²\_{2n}.
Since mean(X) = sum(X_i)/n, this yields an exact confidence interval for θ = 1/λ.

-   For every n, Method 1 produces moderately short intervals.

-   The interval lengths are fairly stable and bell-shaped.

-   For small samples (n=10), lengths are around 2–4.

-   For n=100, they shrink to about 0.5–0.9.

#### Method 2:

It uses the normal approximation of Xˉ, assuming that the variance θ\^2 is known.
...
It tries to solve the inequality from Method 2 for θ, so that the resulting interval no longer depends on the unknown parameter.
...
-   The only time Method 3 performs poorly is when n is very small and α is very small. In this case, the interval does not exist at all (for example, n=5, α=0.01).

#### Method 4:

Method 4 replaces the unknown variance with the sample standard deviation and uses a t-critical value.
For small n, this introduces noticeable variability and undercoverage (around 0.83–0.89 for n=5).
...
The reason is that this method is derived directly from the exact distribution of the statistic 2λn𝑋ˉ, which follows a chi-square distribution with 2n degrees of freedom for exponential data.
...
-   For n=5, coverage is slightly too low (around 0.83–0.89 depending on α).
...
-   For n=100, Method 4 performs almost as well as the exact method.
    ...
    So, Method 4 is a reasonable practical choice when n≥30.

Both Method 2 and Method 3 come from the same normal approximation to the distribution of Xˉ.\
...
-   interval lengths shrink correctly as n increases, Method 3 produces no interval when n is too small relative to α

    Because Method 2 uses the true value of the parameter in the standard error, it is not realistic outside of simulations.
    Method 3 removes θ from the formula, but it gives nearly the same coverage as Method 2 and produces slightly longer intervals.
